...
app.post("/send-cat20", async (req: any, res: any) => {
  try {
    ...
    if (resp.errorCode) {
      // save logs 1
      try {
        saveLogs(withdrawUUID, senderAddress.toString(), receiver.toString(), token.tokenId, token.info.symbol, amount.toString(), resp)
      } catch (error) {
    ...
    // save logs 2:
    try {
      saveLogs(withdrawUUID, senderAddress.toString(), receiver.toString(), token.tokenId, token.info.symbol, amount.toString(), logResp)
    } catch (error) {
...
function saveLogs(withdrawUUID, senderAddress, receivedAddresses, tokenID, symbol, amount, resps) {
